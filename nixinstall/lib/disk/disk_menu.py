...
from nixinstall.lib.disk.encryption_menu import DiskEncryptionMenu
...
class DiskLayoutConfigurationMenu(AbstractSubMenu[DiskLayoutConfiguration]):
	...
	def _prev_btrfs_snapshots(self, item: MenuItem) -> str | None:
		...
		return f'Snapshot type: {snapshot_config.snapshot_type.value}'
	...
	def _prev_disk_encryption(self, item: MenuItem) -> str | None:
		...
		if enc_config:
			...
			output = 'Encryption type' + f': {EncryptionType.type_to_text(enc_type)}\n'
